<script>
    $(document).ready(function() {
        $(".show_modal").click(function() {
            ...
            $.ajax({
                type: "GET",
                url: "alternatif/json/" + id,
                data: {
                    id: id,
                    _token: token
                },
                success: function(data) {
                    $("#modalLabel").html('Detail Alternatif');
                    $("#name").html(data.data[0].alternatif.nama);
                    for (let i = 0; i < data.data.length; i++) {
                        $("#table").append(
                            '<tr>' +
                            '<td style="width: 145px;">' +
                            '<b>' + data.data[i].kriteria.name + '</b>' +
                            '</td>' +
                            '<td style="width: 20px;">:</td>' +
                            '<td>' + data.data[i].nilai + '</td>' +
                            '</tr>'
                        );
                    }
                }
        ...
        $("form[name='alternatif_create']").submit(function(e) {
            e.preventDefault();
            let token = $("input[name=_token]").val();
            var nilai = $("input[name='nilai_kriteria[]']")
                .map(function() {
                    return $(this).val();
                })
                .get();
            var idKriteria = $("input[name='id_kriteria[]']")
                .map(function() {
                    return $(this).val();
                })
                .get();
            $.ajax({
                ...
                data: {
                    _token: token,
                    nama: $("#nama_alternatif").val(),
                    idAlternatif: $("#id_alternatif").val(),
